...
void MainWidget::countOrderAmount()
{
    ...
    if (fCardValue < 0.01) {
        if ((fCardValue * -1) > amount) {
            fCardAmount = amount * -1;
            amount = 0;
        }
        if ((fCardValue * -1) <= amount) {
            amount += fCardValue;
...
void MainWidget::paymentPrint()
{
    ...
    if (QMessageBox::question(this, tr("The question"), tr("Salute?"), QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {
        ...
        if (s.value("taxserver").toInt() == 0) {
            MPTcpSocket ss;
            ss.setServerIP(s.value("taxserverip").toString());
            ss.setValue("order", m_OrderData[ORDER_ID].toString());
            QJsonObject jobj = ss.sendData();
            firm = jobj["firm"].toString();
            hvhh = jobj["hvhh"].toString();
            fiscal = jobj["fiscal"].toString();
            num = jobj["num"].toString();
        } else {
            ...
            if (pt.makeJsonAndPrint(0, 0, in, out, err) != 0) {
                ...
                f.write("\r\n");
                f.close();
                QMessageBox::critical(this, tr("Tax error"), err+ "<br>" + out);
            } else {
    ...
    if (firm.length() == 0) {
        ...
        int l = 30;
        p.addCommand(new QPCText(firm, QPoint(left, top)));
        top += (p.rscale(QFontMetrics(f).height()) / 2) + 5;
        p.addCommand(new QPCText(tr("Tax Number"), QPoint(left, top)));
        p.addCommand(new QPCText(hvhh, QPoint(l, top)));
        top += (p.rscale(QFontMetrics(f).height()) / 2) + 5;
        p.addCommand(new QPCText(tr("Check number"), QPoint(left, top)));
        p.addCommand(new QPCText(num, QPoint(l, top)));
        top += (p.rscale(QFontMetrics(f).height()) / 2) + 5;
        p.addCommand(new QPCText(tr("Fiscal"), QPoint(left, top)));
        p.addCommand(new QPCText(fiscal, QPoint(l, top)));
        top += (p.rscale(QFontMetrics(f).height()) / 2) + 5;
    }
    pen.setWidth(2);
